...
class GoogleSearch(unittest.TestCase):
    ...
    def test_page_and_send_info(self):
            ...
            time.sleep(2)
            name.send_keys('Apurv')
            time.sleep(2)
            email = self.driver.find_element_by_name("Email")
            time.sleep(2)
            email.send_keys('apurvgoyal@abc.com')
            time.sleep(2)
...
if __name__ == '__main__':
    unittest.main(testRunner=HtmlTestRunner.HTMLTestRunner(output='E:/The Sparks Foundation/Project2/Automated_testing_Apurv_goyal/reports'))
